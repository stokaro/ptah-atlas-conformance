...
| `desired-state-workflow` | Do the Atlas desired-state source URLs drive `schema diff`, `schema apply`, and `migrate diff`? | A live SQLite URL works as the `--from` diff source and the `--to` apply source; an atlas.sum-covered migration directory replays on a dev database and is refused before target contact without `--dev-url`; `--to env://src` resolves through atlas.hcl (stokaro/ptah#811). For `migrate diff`, a direct database source writes one integrity-covered migration and converges, `env://url` combines with project dev/dir defaults, and a desired/dev alias is rejected before source mutation or artifact creation (stokaro/ptah#842). |
...
- Some Ptah capabilities are the **local, open half of an Atlas *Pro* feature**
  ...
  continuation contracts; Atlas keeps `migrate checkpoint` in its proprietary
  Pro build, so there is no CE oracle.
  Pre-migration
