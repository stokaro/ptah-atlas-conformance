...
1. Deeper runtime probes for the imported Atlas `.txtar` integration fixtures,
   ...
   resolution. All upstream structural assertions remain active. The only
   translation compiles Atlas-owned diagnostic prose at runtime into ordered
   `(version, group, line, code)` tuples and compares those tuples, including
   multiplicity, with Ptah's report. Legacy URLs/fix blocks, missing or extra
   diagnostics, stderr, and wrong codes remain red without copying prose into
   Ptah or this harness (ptah#651, ptah#747).
