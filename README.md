...
| `migrate-runtime` | Do Atlas-form `migrate ...` commands on the ptah-compat binary preserve Atlas-compatible runtime state against real databases: applied schema objects, Atlas revision rows, `set` repair behavior, transaction-mode rollback/partial-apply semantics, and the same project-config status facts reported by pinned Atlas CE? | the built `ptah-compat` and `ptah` binaries, pinned Atlas CE, live SQLite databases |
