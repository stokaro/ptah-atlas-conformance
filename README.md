...
the remaining offline probes do not open a database. Three further tiers run
against real networked databases in their own workflows. The ORM provider tier
installs external pinned toolchains but does not require a database service.
These tiers stay separate from the deterministic report.
