...
| `migrate-runtime` | Do Atlas-form `migrate ...` commands on the ptah-compat binary preserve Atlas-compatible runtime state against real databases: applied schema objects, Atlas revision rows, stored-state-aware apply/down dry-runs, rollback preflight atomicity, `set` repair behavior, transaction-mode rollback/partial-apply semantics, externally produced `-- atlas:checkpoint` bootstrap-or-skip semantics (including Atlas's own vendored multi-checkpoint directory, so latest-checkpoint selection and post-checkpoint continuation are pinned against upstream-authored files), apply-time `atlas.sum` integrity, and project-config apply equivalence against pinned Atlas CE? | the built `ptah-compat` and `ptah` binaries, pinned Atlas CE, live SQLite/PostgreSQL/MySQL databases |
