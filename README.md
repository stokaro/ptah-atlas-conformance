...
| `txtar-script` | Does the harness parse Atlas integration txtar scripts, execute the command subset currently mapped to Ptah APIs, publish the fixture-level script surface, and keep unsupported runtime commands red? The sqlite `cli-migrate-lint-*` fixtures run end to end through the real `atlas migrate lint` CLI (the ptah-compat binary) against ephemeral SQLite. Every upstream structural assertion remains active. Atlas-owned diagnostic prose is compiled at runtime into ordered `(version, group, line, code)` tuples and compared exactly with Ptah's report, including multiplicity; legacy URLs/fix blocks, missing/extra diagnostics, stderr, and wrong codes remain red without copying prose into Ptah or this harness. | harness, `core/parser`, `core/renderer`, the built `ptah-compat` binary (`migrate lint`) |
