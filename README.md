...
| `txtar-script` | Does the harness parse Atlas integration txtar scripts, execute the command subset currently mapped to Ptah APIs, publish the fixture-level script surface, and keep unsupported runtime commands red? The sqlite `cli-migrate-lint-*` fixtures run end to end through the real `atlas migrate lint` CLI (the ptah-compat binary) against ephemeral SQLite. The harness evaluates the assertions in the upstream scripts directly against the actual CLI streams and files; it does not translate Atlas prose into Ptah-specific semantic codes or maintain a second expected-output model. | harness, `core/parser`, `core/renderer`, the built `ptah-compat` binary (`migrate lint`) |
...
| `composite-schema-workflow` | Do independently owned Go and YAML desired-schema sources render exactly like a committed hand-merged schema, reject conflicting identities, generate identical up/down migrations, and converge on verified SQLite schema facts? Render checks compare the complete executable-SQL stdout, with progress output confined to stderr. | the built `ptah` binary, `core/goschema`, ephemeral SQLite |
...
| `checkpoint-workflow` | Does `ptah migrations checkpoint` squash a migration history into a deterministic cumulative-schema pair covered by `ptah.sum`, from which a fresh database bootstraps to a schema structurally identical to the full replay while an already-migrated database ignores it — with tamper detection (`validate` exit 1), a below-boundary rollback refusal (exit 2), a working rollback-to-zero, and post-checkpoint continuation? Atlas keeps the checkpoint-*generating* `migrate checkpoint` command in its proprietary Pro build, so there is no CE oracle for producing one — but the pinned Atlas CE does honor the `-- atlas:checkpoint` directive on *read*, and the `migrate-runtime` tier measures that half against it. | the built `ptah` binary, `migration/generator`/`migration/migrator`, ephemeral SQLite |
| `external-schema-workflow` | Do static SQL and external SQL/HCL/YAML desired schemas preserve the trust boundary and drive render, compare, drift, plan, generate, apply, live SQLite facts, and converged no-op results through the real Ptah CLI? Render snapshots consume the complete executable-SQL stdout rather than a legacy display header. | the built `ptah` binary, external fixture provider, ephemeral SQLite |
...
| `pro-plan-workflow` | Does the local half of Atlas's Pro `schema plan` workflow hold: the parent command and `schema plan new` write fingerprinted local plans, `schema plan validate` accepts a fresh plan and rejects a stale one without mutating the target, `schema apply --plan file://...` replays the reviewed plan, and stale apply is refused? Atlas binds remote plan storage and approval to its Cloud registry. | the built `ptah` binary (`atlas schema plan`, `schema plan new`, `schema plan validate`, and `schema apply --plan`), ephemeral SQLite |
| `pro-down-workflow` | Do real default-output and `--format` `atlas migrate down` runs — no stdin, Ptah-only `--confirm`, or `--revision-format` flag — read the Atlas-format revision rows `atlas migrate apply` wrote and actually revert (stokaro/ptah#810 and #971; previously a silent no-op against Ptah's native revision table or an interactive rollback)? | the built `ptah` binary (`atlas migrate apply`/`down`), ephemeral SQLite |
| `desired-state-workflow` | Do the Atlas desired-state source URLs work for `schema diff`, `schema apply`, and `migrate diff` (stokaro/ptah#811, stokaro/ptah#842): live database URLs, migration-directory replay, evaluated `env://` references, a generated directory that validates, replays, and converges, and destructive desired/dev path-alias rejection before source mutation or artifact creation? | the built `ptah-compat` binary (`schema diff`/`apply`, `migrate diff`/`validate`), ephemeral SQLite |
...
validation, and return the same exit code and stdout/stderr after tampering.
Atlas CE appends a community-build installation advisory only to the first
error in a 24-hour HOME state window. The probe uses an isolated HOME, records
that first-error advisory exactly as a separate Atlas observation, then compares
Ptah with Atlas's stable repeated-error checksum contract instead of allowing a
developer's pre-existing Atlas state to decide the report.

A second Goose differential covers reading a **converted** directory, the
capability stokaro/ptah#1002 added: `migrate status` and `migrate set` accept a
directory laid out in a foreign tool's convention instead of refusing the
layout. Atlas CE and ptah-compat each hash their own copy of the same Goose
directory, then run `set` and `status` against it. `set` is compared byte for
byte; `status` is compared on the migration facts (total/applied/pending),
because the two binaries render the status block differently for **every**
directory format and byte equality there would measure that unrelated
difference instead. The probe also pins that the format name is taken verbatim:
both binaries refuse `Goose`, `GOOSE`, ` goose`, and `goose ` with exit 1 and
echo the rejected value, so Ptah cannot end up quietly more permissive than the
binary it stands in for. The offline tier pins the same capability through both
spellings a caller can use — the `--dir-format` flag and a `format` query
parameter on the `--dir` URL.
...
known Atlas release rather than a moving target. Differential, CE-gating, and
CLI-surface generators reject an `ATLAS_BIN` whose Community version does not
match that pin. The differential is scoped to the object kinds Atlas CE can
actually inspect: tables, columns, constraints, indexes and enums.
...
CE binary for its independent project-config apply oracle. Set PostgreSQL,
MySQL, and MariaDB URLs to include the networked runtime contours:
...
- per-verb expectations for the out-of-scope (Pro/Cloud) commands: verbs Ptah
  ...
  `migrate edit`/`rebase`/`rm`, `migrate down`, `schema plan`, `schema plan
  new`, `schema plan validate`, `migrate checkpoint`) **must**
  ...
  verbs (`migrate push`, `schema push`, the seven Cloud-only `schema plan`
  registry sub-verbs)
