...
| `migrate-runtime` | Does `ptah atlas migrate ...` preserve Atlas-compatible runtime state against real databases: applied schema objects, Atlas revision rows, `set` repair behavior, transaction-mode rollback/partial-apply semantics, and the same project-config status facts reported by pinned Atlas CE? | the built `ptah` binary, pinned Atlas CE, live SQLite databases |
...
| `desired-state-workflow` | Do the Atlas desired-state source URLs work for `schema diff`/`schema apply` (stokaro/ptah#811) and `migrate diff` (stokaro/ptah#842): live database URLs, migration-directory replay, `env://` resolution, migrate-diff convergence, and desired/dev alias rejection before source mutation or artifact creation? | the built `ptah` binary (`atlas schema diff`/`apply`, `atlas migrate diff`), ephemeral SQLite |
...
| `migrate-runtime` | [`conformance-migrate-runtime`](./.github/workflows/conformance-migrate-runtime.yml) | Do real `ptah atlas migrate apply/status/set` executions leave the same supported migration state Atlas callers rely on? This tier inspects schema objects and `atlas_schema_revisions` rows directly. For project configuration, Atlas CE creates the brownfield revision state and both CLIs must report identical typed available/applied/pending/current/next/status facts from the same untouched `atlas.hcl`. CI also covers SQLite `--revisions-schema main` and `--tx-mode all/file/none`, PostgreSQL custom revision schemas and `CREATE INDEX CONCURRENTLY` with `atlas:txmode none`, and MySQL apply/status revision state. | built `ptah` binary; pinned Atlas CE (`ATLAS_BIN`); local SQLite databases; `CONFORMANCE_POSTGRES_URL` / `CONFORMANCE_MYSQL_URL` |
...
CE binary for its independent project-config status oracle. Set Postgres/MySQL
