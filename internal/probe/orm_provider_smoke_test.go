...
func TestORMProviderSmokeProbe_HappyPath(t *testing.T) {
	...
	c.Assert(results, qt.DeepEquals, []probe.Result{
		...
		},
		{
			...
			Stage:   "provider output",
			Outcome: probe.OK,
			Detail:  "SQLAlchemy provider 0.5.0 with SQLAlchemy 2.0.52 provider output preserved two tables, primary keys, a unique index, and a foreign key",
		},
		{
			Probe:   "orm-provider-smoke",
			Fixture: "sqlalchemy",
			Stage:   "ptah schema render",
			Outcome: probe.OK,
			Detail:  "SQLAlchemy provider 0.5.0 with SQLAlchemy 2.0.52 ptah schema render preserved two tables, primary keys, a unique index, and a foreign key",
		},
	})
