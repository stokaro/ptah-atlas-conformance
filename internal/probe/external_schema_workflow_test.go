...
func TestExternalSchemaWorkflowProbe_BrokenExpectedRenderTurnsRed(t *testing.T) {
	...
	c.Check(results[0].Detail, qt.Contains, "rendered SQL differs from the expected SQLite snapshot (-want +got):")
	c.Check(results[0].Detail, qt.Contains, "-- deliberately wrong")
	c.Check(results[0].Detail, qt.Contains, "CREATE TABLE")
