...
func txtarWritePostgresTableShowSQL(
	...
	for _, column := range table.Columns {
		...
		if column.AutoInc {
			...
			defaultValue = atlasDefaultSQL("postgresql", column.Default)
		}
		fmt.Fprintf(out, "%s | %s |  | %s | %s\n",
			atlasSQLIdentifier(column.Name),
			atlasColumnType("postgresql", column.Type),
			nullable,
			defaultValue,
		)
	}
...
func txtarPostgresShowIndexLines(tableName string, table *ast.CreateTableNode, indexes []*ast.IndexNode) []string {
	...
	for _, index := range indexes {
		...
		lines = append(lines, fmt.Sprintf("%q %sbtree (%s)", atlasSQLIdentifier(index.Name), kind, strings.Join(parts, ", ")))
	}
	return lines
}
